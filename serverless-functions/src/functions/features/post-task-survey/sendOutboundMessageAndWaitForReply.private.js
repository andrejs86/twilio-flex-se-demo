...
} = require(Runtime.getFunctions()['features/post-task-survey/helpers/conversations_helper'].path);
...
exports.sendOutboundMessageAndWaitForReply = async (
  ...
  if (KnownAgentRoutingFlag) await setKnownAgentRoutingOnConversationAttributes(client, conversation, WorkerSid);

  // Point a webhook at studio to handle replies from customer
  if (!reusingExistingConversation) {
    await setStudioWebhookOnConversation(client, conversation, InboundStudioFlow);
  }

  // Add agents initial message
  await addAgentMessageToConversation(client, conversation, WorkerFriendlyName, Body, ContentTemplateId);
